...
function assertFhevmStoredPublicKey(
  ...
  if (typeof v.publicKeyId !== "string") throw new Error(`FhevmStoredPublicKey.publicKeyId must be a string`);
  if (!(v.publicKey instanceof Uint8Array)) throw new Error(`FhevmStoredPublicKey.publicKey must be a Uint8Array`);
}
...
function assertFhevmStoredPublicParams(
  ...
  const v = value as Record<string, unknown>;
  if (typeof v.publicParamsId !== "string") throw new Error(`FhevmStoredPublicParams.publicParamsId must be a string`);
  if (!(v.publicParams instanceof Uint8Array)) throw new Error(`FhevmStoredPublicParams.publicParams must be a Uint8Array`);
}
