...
// Check if FHEVM SDK is already initialized
...
export const createFhevmInstance = async (parameters: {
  ...
  // Save the key - getPublicKey() and getPublicParams() return the correct types
  const publicKey = instance.getPublicKey();
  const publicParams = instance.getPublicParams(2048);
  
  await publicKeyStorageSet(
    aclAddress,
    publicKey as { publicKeyId: string; publicKey: Uint8Array } | null,
    publicParams as { publicParamsId: string; publicParams: Uint8Array } | null
  );
